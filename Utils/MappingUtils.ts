...
export function mapSearchPostResponse(post: Post, email: string): SearchPostDto {
    return {
        msg: null,
        post: {
            postId: post?.id,
...
export function mapGetUsersResponse(usersResponse: User[]): UsersDataResponseDto {
...
export function mapGetUserPostsResponse(userData: User, postsData: Post[]): UserPostsResponseDto {
    return postsData
        ?.filter((post) => !!post)
